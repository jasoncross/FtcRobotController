...
- INIT now runs a Limelight pipeline auto-selection pass in both TeleOp and Auto. The selector reads the tunable `LIMELIGHT_PIPELINE_PROFILES` list, switches through each pipeline (with settle + multi-frame sampling delays), and scores them by AprilTag precedence: **alliance goal tag → opposing goal tag → obelisk tag → none**. The chosen pipeline is locked for the rest of the match so no further auto switching occurs.
- If no relevant tags are seen, auto-selection falls back to pipeline **0 – Meet Gym**, locks it, and surfaces an **urgent** first-line banner: `LL AUTOSELECT FALLBACK -> 0 - Meet Gym (reason=...)`. Successful selections—including ties—use normal telemetry, always showing `LL Profile: <index> - <description>` alongside other vision status lines.
