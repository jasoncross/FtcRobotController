...
- Limelight odometry fusion now filters botpose localization to a configurable goal-tag whitelist (default 20/24) using the fiducial override call so obelisk AprilTags never influence pose, while aim/RPM targeting stays alliance-specific through the Limelight priority ID.
