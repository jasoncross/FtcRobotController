...
- Goal-aim locking now uses **per-fiducial tx/tz from the alliance goal tag only (BLUE 20 / RED 24)** with stale windows, tx hysteresis, and confirmation frames controlled via `VisionConfig.AimLock.*`; when the goal drops, aim stops instead of jumping to obelisk tags, and telemetry reports locked vs. global tx plus lock age. Even if `LLResult.isValid()` is false, a fiducial entry for the alliance goal still feeds heading/range so Tag Visible telemetry matches what Limelight actually sees.
