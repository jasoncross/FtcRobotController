...
- INIT now runs a Limelight pipeline auto-selection pass in both TeleOp and Auto. The selector reads the tunable `LIMELIGHT_PIPELINE_PROFILES` list, switches through each pipeline (with settle + multi-frame sampling delays), and scores them by AprilTag precedence: **alliance goal tag → opposing goal tag → none**. The chosen pipeline is locked for the rest of the match so no further auto switching occurs.
