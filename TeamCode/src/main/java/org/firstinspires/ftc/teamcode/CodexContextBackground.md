...
- FeedStop homing is now queued until START so INIT stays motionless; the servo homes and parks at the blocking angle as soon as
  the match begins.
- FeedStop release is gated by the launcher RPM window, so TeleOp feeds/ejects and Auto continuous-fire steps only open the gate
  once the flywheels are within the shared tolerance for the configured settle time.
