...
- AUTO now applies alliance-only goal filtering with Limelight-side hysteresis (multi-frame acquire/loss counters and a short tx hold) so single-frame dropouts no longer flip between scan and aim; BaseAuto telemetry surfaces raw vs. smoothed visibility, held tx, lost-frame count, and pipeline index for verification.
 - Goal-aim locking now uses **per-fiducial tx/tz from the alliance goal tag only (BLUE 20 / RED 24)** with stale windows, tx hysteresis, and confirmation frames controlled via `VisionConfig.AimLock.*`; when the goal drops, aim stops instead of jumping to obelisk tags, and telemetry reports locked vs. global tx plus lock age. Even if `LLResult.isValid()` is false, a fiducial entry for the alliance goal still feeds heading/range so Tag Visible telemetry matches what Limelight actually sees. Goal detection now keys off fiducial presence while aim validity still requires a finite tx sample; telemetry lists observed IDs plus the raw goal tx (even when null) and a finite/valid flag to spot tx dropouts in the field.
