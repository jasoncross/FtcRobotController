...
- MT2 localization pulls the deterministic **blue-origin** pose stream via LimelightHelpers (no `LLResult.getBotpose_MT2()` fallback), so frames stay consistent across updates.
...
- Limelight MT2 botpose streams are corner-origin; odometry shifts them to the FTC center-origin frame by subtracting 72 in from both axes after axis swap/sign correction and before applying calibration offsets.
