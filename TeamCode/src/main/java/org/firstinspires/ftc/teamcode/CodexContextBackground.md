...
- Limelight odometry fusion now filters botpose localization to the two goal tags (20/24 only) using the MT2 fiducial override call so obelisk AprilTags never influence pose, while aim/RPM targeting stays alliance-specific through the Limelight priority ID.
...
- Limelight MT2 localization now uses the explicit LimelightHelpers PoseEstimate stream (`getBotPoseEstimate_wpiBlue_MegaTag2`) to avoid frame ambiguity from `LLResult.getBotpose_MT2()`. The wpiBlue stream is treated as the single pose origin for localization.
- Corner-origin Limelight poses are shifted to the FTC center-origin frame by subtracting 72 in from both axes when `APPLY_CENTER_SHIFT` is enabled (after axis swap/sign and before offsets).
...
- TeleOp now appends a single `VisionDbg` telemetry line showing yaw feed status/age, localization filter status/age, MT2 usage, raw vs. accepted vs. fused pose, and the last rejection reason so crews can diagnose pose drift quickly. Temporary fields `rawBlue` and `rawRed` display MT2 blue/red origin poses (offsets disabled) to compare frame alignment.
