...
- MegaTag2 yaw is written each loop to the **root** Limelight NetworkTables table (`"limelight"`) at `robot_orientation_set`; `yawOk` in the `VisionDbg` line confirms the write executed and `yawSent` reports the value sent.
