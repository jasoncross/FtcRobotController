...
| --- | --- |
| `BaseDriveTeleOp` | Creates basic drivetrain, gamepad bindings, rumble/aim controllers, and optional vision. Updates vision during INIT without driving. Constructs ReverseDriveControl after START, seeding the held-button gate without firing a callback. During RUN, reads controls, selects the configured target, optionally applies aim twist, and commands drive using DriveInputScaling for trigger braking. Gates aim feedback only while ToggleRumble reports a programmed confirmation in progress. Stops motors/rumble and closes vision in `finally`. |
...
| `ReverseDriveControl` | Per-TeleOp normal/reverse state. Registers the G1 left-stick press through ControllerBindings, inverts only forward/strafe, and confirms ON with two rumble pulses and OFF with one. Instances constructed at START begin OFF; an already-held button must be released before the next press can toggle. Delegates confirmation playback to ToggleRumble. |
...
| `RumbleNotifier` | Nonblocking heading-error feedback with configurable envelope. TeleOp Y toggles feedback and plays the configured confirmation pattern. The caller controls confirmation priority; RumbleNotifier itself only owns the aim envelope and cooldown. |
| `ToggleRumble` | Shared by reverse and Y-toggle callbacks. Builds confirmations from TeleOpRumbleTuning and tracks their programmed duration on a monotonic clock. Ordinary aim pulses do not extend this deadline; BaseDriveTeleOp uses it to defer aim feedback. This timer is not acknowledgement of physical playback. |
| `DriveInputScaling` | Pure trigger-to-speed calculation used by BaseDriveTeleOp; clamps trigger and configured floor to 0–1 and returns a fraction of the drivetrain cap. |
