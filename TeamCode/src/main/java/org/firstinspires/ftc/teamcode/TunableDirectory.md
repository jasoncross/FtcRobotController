...
| `VisionTuning.VISION_RES_WIDTH` / `VISION_RES_HEIGHT` | `config/VisionTuning.java` | Both | Resolution passed to the VisionPortal builder. | Update together so TeleOp + Auto stream the same pixel dimensions; TeleOp telemetry reflects current values. | 1280×720 for long-range AprilTags; drop to 640×480 for lighter pipelines. |
| `VisionTuning.VISION_TARGET_FPS` | `config/VisionTuning.java` | Both | Target FPS limit applied (if supported) to the AprilTag processor. | If the SDK ignores FPS limits the code falls back gracefully; adjust here first before experimenting in code. | 20 FPS balances 720p clarity with Control Hub throughput. |
| `VisionTuning.APRILTAG_DECIMATION` | `config/VisionTuning.java` | Both | Decimation factor applied to AprilTag frames. | TeleOp pushes the value into the AprilTagProcessor during init; Auto inherits automatically. | `2.0` to lighten CPU at 720p; lower when tags are distant. |
| `VisionTuning.EXPOSURE_MS` | `config/VisionTuning.java` | Both | Manual exposure applied via camera controls (ms). | TeleOp attempts to set the value once streaming; telemetry warns if unsupported. Adjust with caution to avoid overexposure. | `15 ms` for Control Hub indoor lighting; raise slightly in darker venues. |
| `VisionTuning.GAIN` | `config/VisionTuning.java` | Both | Manual gain applied via camera controls. | Shared for TeleOp/Auto; telemetry issues a one-time warning if the webcam ignores the setting. | `110` for Logitech C270 at 720p; lower to reduce noise. |
| `VisionTuning.WHITE_BALANCE_LOCK_ENABLED` | `config/VisionTuning.java` | Both | Whether to lock the camera’s white balance after streaming starts. | TeleOp attempts to lock via `WhiteBalanceControl` or falls back silently if unsupported. | `true` for consistent color temperature; set `false` outdoors. |
