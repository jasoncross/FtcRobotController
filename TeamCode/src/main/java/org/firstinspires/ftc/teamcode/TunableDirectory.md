...
| `FeedStopConfig.SCALE_MIN` | `config/FeedStopConfig.java` | Both | Lower bound sent to `ServoImplEx.scaleRange(...)`; narrowing the window shortens travel for quicker blocking. | Tune alongside `SCALE_MAX` so both ends stay balanced; adjust only here so TeleOp and Auto share the new range. | Raise toward `0.45` if the servo can stop sooner; drop toward `0.40` when the gate needs a deeper block. |
| `FeedStopConfig.SCALE_MAX` | `config/FeedStopConfig.java` | Both | Upper bound for the scaled servo range. | Keep paired with `SCALE_MIN` when widening/narrowing overall swing; no other file overrides it. | Lower toward `0.56` to reduce motion; raise toward `0.60` if the release arm needs more travel. |
| `FeedStopConfig.BLOCK_POS` | `config/FeedStopConfig.java` | Both | Normalized block position inside the scaled range. | Adjust here after hardware tweaks; Feed uses this value everywhere. | Shift slightly above `0.0` if the servo binds at the hard stop. |
| `FeedStopConfig.RELEASE_POS` | `config/FeedStopConfig.java` | Both | Normalized release position inside the scaled range. | Change here to align with launcher clearance; Feed reads it directly. | Drop to `0.95` if the gate opens too far; push toward `1.00` (with matching scale tweak) for extra clearance. |
| `FeedStopConfig.RELEASE_HOLD_MS` | `config/FeedStopConfig.java` | Both | Time the gate stays at RELEASE after a fire request before returning to BLOCK. | Shared by TeleOp + Auto; update here to change hold duration for all fire routines. | Extend to `325 ms` if artifacts linger; trim toward `200 ms` when recovery needs to start sooner. |
