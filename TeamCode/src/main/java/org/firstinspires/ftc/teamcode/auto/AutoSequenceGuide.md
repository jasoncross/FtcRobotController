...
### Creating a New Auto OpMode

Students can spin up a fresh autonomous class in just a few steps:

1. **Copy an existing template.** Duplicate one of the current autos in
   [`auto/`](.) and rename it (for example, `Auto_Blue_Wing.java`).
2. **Update the file header.** Change the `PURPOSE`, tunable notes, and
   `CHANGES` comments so they describe the new path. Use today’s date for
   any new changelog lines.
3. **Adjust the class annotation.** Update the
   `@Autonomous(name=..., group=..., preselectTeleOp=...)` values so it
   appears correctly on the Driver Station list and preselects the proper
   TeleOp.
4. **Declare the alliance.** Override `alliance()` to return either
   `Alliance.BLUE` or `Alliance.RED`. This controls AprilTag IDs, scan
   direction defaults, and telemetry color accents.
5. **Describe the start pose.** Override `startPoseDescription()` with the
   exact tile/orientation instructions you expect your drive team to use
   (see [Updating Start Pose Telemetry](#updating-start-pose-telemetry)).
6. **Script the sequence.** Implement `runSequence()` with the fluent
   calls shown below, finishing with `.run();`. Include any `spinToAutoRpm`
   warm-ups, `aim` steps, or `fire` volleys needed for the route.
Here is a minimal skeleton you can adapt:

```java
@Autonomous(name = "Auto: Blue Wing", group = "Auto", preselectTeleOp = "TeleOp - Blue")
public final class Auto_Blue_Wing extends BaseAuto {
    @Override protected Alliance alliance() { return Alliance.BLUE; }

    @Override protected String startPoseDescription() {
        return "Start: Blue Wing — West launch tile, FACING NORTH";
    }

    @Override protected void runSequence() throws InterruptedException {
        sequence()
                .move("Drive to wing", 28.0, 0.0, 0.55)
                .spinToAutoRpm("Warm up launcher")
                .rotateToTarget("Acquire tag", 2500, null)
                .aim("Spin launcher", 3200)
                .fire("Score preload", 3, true, 3000)
                .run();
    }
}
```

Add the new file to the README project layout and capture the change in
the revision history when you commit.
...
| `returnToStoredHeading(label, speedCap)` | Turns back to the most recent recorded heading. | No-op if `rememberHeading()` was never called. Honors the same twist caps as other turn helpers. |
...
| `rotateToTarget(label, timeoutMs, clockwiseFirst)` | Sweeps for the alliance goal AprilTag until lock tolerance is satisfied or timeout elapses. | Uses `TagAimController` + shared lock tolerance. Pass `null` to reuse BaseAuto’s alliance default; otherwise `clockwiseFirst` sets the initial sweep direction. |
...
| `eject(label)` | Runs the TeleOp-style eject routine once. | Spins the launcher at `TeleOpEjectTuning.RPM`, feeds with intake assist, then restores the prior AutoSpeed hold so the launcher stays warm. |
...
| `custom(label, action)` | Runs arbitrary code. | `action` is a `Runnable` executed synchronously; use sparingly for bespoke logic (see [`.custom(...)` examples](#5-using-custom-for-special-cases)). |
...
### 4. Clearing an Artifact Jam Mid-Sequence

```java
sequence()
    .waitFor("Pause to inspect", 250)
    .eject("Clear artifact")
    .spinToAutoRpm("Re-spin launcher")
    .run();
```

**When to use:** Driver Station telemetry shows an intake jam or a stuck
artifact during testing. `eject(...)` mirrors the TeleOp eject button,
including intake assist timing, and automatically restores the previous
AutoSpeed RPM so you can resume the route immediately.

---

### 5. Using `.custom(...)` for Special Cases

Because `.custom(...)` drops a lambda directly into the step list, it is
best reserved for logic that does not warrant a dedicated helper yet. A
few proven patterns:

- **Telemetry or dashboard breadcrumbs.** Emit a one-off log packet to
  mark field observations, such as which spike mark or AprilTag ID the
  robot just confirmed.
- **One-off actuators.** Pulse a mechanism that only exists on your
  robot (for example, an LED ring or camera turret) before resuming the
  normal drivetrain/launcher helpers.
- **Conditional checks.** Inspect a sensor (beam break, color, limit
  switch) and branch accordingly—possibly calling `stopAll()` or setting
  flags that later steps read.
- **Temporary diagnostics.** During tuning, expose subsystem state by
  calling debug dump helpers; remove the step once the issue is solved
  so competition code stays clean.

Each call still runs inside the OpMode’s `opModeIsActive()` guard, so
these hooks inherit the same safety envelope as the stock builder steps.
...
## Updating Start Pose Telemetry

The `Phase` banner in Driver Station always shows the string returned by
`startPoseDescription()` from your auto class. Update it whenever you
change the physical staging tile or robot orientation:

1. Open the auto class (for example,
   [`auto/Auto_Blue_Human.java`](./Auto_Blue_Human.java)).
2. Edit the `startPoseDescription()` override so the text matches the new
   setup instructions (“Start: Blue Human — West tile, FACING NORTH”).
3. Deploy and run the routine; confirm the INIT telemetry now prints the
   revised wording under **Start Pose**.

Keeping this text aligned with your drive team’s staging plan prevents
misplaced robots from undermining a dialed-in AutoSequence.
