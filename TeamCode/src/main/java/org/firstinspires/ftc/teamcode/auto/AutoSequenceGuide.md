...
| `fireContinuous(label, timeMs, requireLock)` | Holds the FeedStop open and streams feed power until `timeMs` elapse. | Keeps AutoSpeed enabled so launcher RPM stays latched; when `requireLock` is true the step is skipped if the prior aim step never achieved lock. |
