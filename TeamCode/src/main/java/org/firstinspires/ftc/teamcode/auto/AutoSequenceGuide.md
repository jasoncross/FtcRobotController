...
| `adjustAutoScale(label, percentage)` | Applies a percentage tweak to AutoSpeed outputs for the rest of AUTO (non-blocking). | Mirrors the TeleOp D-pad AutoRPM tweak by multiplying the AutoSpeed target (for example, `0.02` = +2%). Clamped to 0.50–1.50× overall so extreme scales are bounded. |
