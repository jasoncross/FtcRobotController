...
public abstract class BaseAuto extends LinearOpMode {
    ...
    //                        reasserted AutoSpeed targets during volleys, extended AutoSequence to
    //                        pre-spin flywheels ahead of tag locks, and aligned hold-target recovery naming.
    ...
    protected final void fireN(int count, boolean requireLock, long betweenShotsMs) throws InterruptedException {
        ...
        for (int i = 0; i < count && opModeIsActive(); i++) {
            ...
            if (holdRpmTarget <= 0) {
                holdRpmTarget = launcher.targetRpm;
                if (holdRpmTarget <= 0) {
                    holdRpmTarget = autoSeedRpm();
                }
            }
            launcher.setTargetRpm(holdRpmTarget);
