...
/**
 * Encoder/IMU mecanum motion helpers retained from the final DECODE robot.
 * Geometry, gains, taper and stall tuning remain in DriveTuning. Confirm calibration
 * before constructing this class. Motor names/directions come from RobotConfig.
 * Positive drive is forward and positive strafe is right, matching MecanumDrive.
 * IMU headings and turn() angles are CCW; moveWithTwist field directions are CW.
 * Basic robot-centric driving can use MecanumDrive without enabling these helpers.
 */
public class Drivebase {

    // ======= TUNE THESE =======
    public static final double WHEEL_DIAMETER_IN = DriveTuning.WHEEL_DIAMETER_IN; // Historical effective wheel diameter; review DriveTuning
    public static final double TICKS_PER_REV     = DriveTuning.TICKS_PER_REV;     // Encoder ticks per motor revolution
    public static final double GEAR_RATIO        = DriveTuning.GEAR_RATIO;        // motor output revs per wheel rev
    ...
    public Drivebase(LinearOpMode op) {
        ...
        imu  = hw.get(IMU.class, RobotConfig.IMU_NAME);
        fl   = hw.get(DcMotorEx.class, RobotConfig.FRONT_LEFT);
        fr   = hw.get(DcMotorEx.class, RobotConfig.FRONT_RIGHT);
        bl   = hw.get(DcMotorEx.class, RobotConfig.BACK_LEFT);
        br   = hw.get(DcMotorEx.class, RobotConfig.BACK_RIGHT);
    ...
    private void commonInit(boolean teleOp) {
        ...
        fl.setDirection(RobotConfig.FRONT_LEFT_DIRECTION);
        bl.setDirection(RobotConfig.BACK_LEFT_DIRECTION);
        fr.setDirection(RobotConfig.FRONT_RIGHT_DIRECTION);
        br.setDirection(RobotConfig.BACK_RIGHT_DIRECTION);
    ...
    public void move(double distanceInches, double degrees, double speed) {
        final double actionStartedAt = now();
        try {
            ...
            while (isActive() && now() - actionStartedAt < DriveTuning.AUTO_ACTION_TIMEOUT_SEC) {
    ...
    public void moveWithTwist(double distanceInches,
        ...
        try {
            ...
            while (isActive() && now() - actionStartedAt < DriveTuning.AUTO_ACTION_TIMEOUT_SEC) {
                ...
                double[] robotVector = MecanumMixer.fieldVector(degrees, curHeading);
                double forwardRaw = robotVector[0];
                double lateralRaw = robotVector[1];
                double forward = forwardRaw;
                ...
                double twistCmd = -(TURN_KP * err + TURN_KD * derr); // Convert CCW heading correction to CW drive power
            ...
            if (isActive() && now() - actionStartedAt < DriveTuning.AUTO_ACTION_TIMEOUT_SEC && abs(residual) > TURN_TOLERANCE_DEG) {
    ...
    // --- op-mode context ---
    private static void requireCalibration() {
        if (!DriveTuning.CALIBRATION_CONFIRMED) {
            throw new IllegalStateException("Review DriveTuning, motor directions and IMU mounting, then confirm calibration");
        }
        if (!Double.isFinite(DriveTuning.AUTO_ACTION_TIMEOUT_SEC) || DriveTuning.AUTO_ACTION_TIMEOUT_SEC <= 0) {
            throw new IllegalArgumentException("AUTO_ACTION_TIMEOUT_SEC must be positive and finite");
        }
    }
