...
## Revision History
- **2026-01-09** – Ensured AUTO always reasserts the intake-on default at START, immediately syncs the firing controller’s desired intake state, restores intake after each auto firing sequence, and added MAIN/ENDGAME timing reserves plus immediate ENDGAME sequencing once MAIN steps complete so human-side autos can bail into final retreat moves before the 30 s hard stop.
