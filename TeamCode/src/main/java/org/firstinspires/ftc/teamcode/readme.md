...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls (reflection on `SetRobotOrientation`/`setRobotOrientation`), added yaw age telemetry, and hardened localization tag filters with LimelightHelpers override calls plus success/age diagnostics so pose fusion stays locked to goal tags without WPILib dependencies.
