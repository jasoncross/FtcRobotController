...
## Revision History
- **2025-12-29** – Reworked Limelight MT2 yaw feeding to use `LimelightHelpers.setRobotOrientation(...)` each loop with telemetry confirmation, enforced goal‑tag‑only localization via `LimelightHelpers.setFiducialIDFilters(...)` (20/24), added field-bounds gates, restored Drivebase encoder distance math to physical counts, and added an odometry-only distance scale plus `OdoDbg` telemetry to keep Auto moves accurate while pose calibration remains adjustable.
