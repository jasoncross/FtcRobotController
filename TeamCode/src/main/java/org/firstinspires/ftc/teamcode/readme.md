...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls, applied MT2 fiducial filter overrides for goal-only localization, standardized localization on `LLResult.getBotpose()` with a guarded MT2 getter fallback, added `mt2Exp/mt2Act` + pose source/raw meters telemetry, and kept the corner→center shift configurable while locking axis mapping (`AXIS_SWAP_XY=true`, `X_SIGN=+1`, `Y_SIGN=-1`) so transforms stay in field coordinates.
