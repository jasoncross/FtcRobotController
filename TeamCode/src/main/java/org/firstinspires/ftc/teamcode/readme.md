...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls, applied MT2 fiducial filter overrides for goal-only localization, standardized localization on `LLResult.getBotpose()` with a guarded MT2 getter fallback, added `mt2Exp/mt2Act` + pose source/raw meters telemetry, and moved BOUNDS rejection to test the final candidate pose against `FIELD_HALF_IN - BOUNDS_MARGIN_IN` with explicit telemetry for the tested values.
