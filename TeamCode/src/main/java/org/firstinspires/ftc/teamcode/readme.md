...
- **2025-12-16** – Added Limelight goal-visibility hysteresis for AUTO (alliance-only goal filter, acquire/loss frame counters, and a 150 ms tx hold), routed TagAim to the smoothed heading sample, and surfaced raw vs. smoothed visibility, tx-to-use, lost-frame counts, and active pipeline index in AUTO telemetry to stop scan jitter on single-frame dropouts.
