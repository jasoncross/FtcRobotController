...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls, applied MT2 fiducial filter overrides for goal-only localization, switched localization to the LimelightHelpers PoseEstimate wpiBlue MT2 stream, added `rawBlue`/`rawRed` debug frame readouts plus yaw/filter age telemetry in `VisionDbg`, and made the corner→center shift configurable (`FIELD_HALF_IN`, `APPLY_CENTER_SHIFT`) while locking axis mapping (`AXIS_SWAP_XY=true`, `X_SIGN=+1`, `Y_SIGN=-1`) and keeping offsets at `0.0` pending revalidation.
