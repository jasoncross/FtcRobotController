...
## Revision History
- **2025-12-29** – Reworked Limelight MT2 yaw feeding to write `robot_orientation_set` on the root `"limelight"` table each loop with telemetry confirmation, enforced goal‑tag‑only localization filters (20/24) with periodic reassertion, added field-bounds gates, and appended a `VisionDbg` line showing raw/accepted/fused poses plus rejection reasons while keeping aim/RPM alliance-priority locked.
