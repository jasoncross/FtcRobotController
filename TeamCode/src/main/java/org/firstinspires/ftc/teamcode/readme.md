...
- **Per-tag locking:** Limelight aim now locks exclusively to the alliance goal tag (BLUE 20 / RED 24) using **per-fiducial tx/tz** samples—global tx/ty/ta are ignored for control. The lock clears when the goal disappears (with a brief hold controlled by `VisionConfig.AimLock.AIM_LOCK_STALE_MS`), requires improvement hysteresis + confirmation frames (`AIM_SWITCH_TX_HYST_DEG`, `AIM_SWITCH_CONFIRM_FRAMES`), and surfaces locked vs. global tx plus lock age in telemetry so obelisk tags can never steal aim.
- **Telemetry consistency:** The TeleOp top-line “Tag Visible” status now always reflects the alliance goal’s heading/range using the live fiducial sample even if the lock freshness timer lapses, keeping the display aligned with goal-only aim inputs.
...
## Revision History
- **2025-12-19** – Ensured TeleOp goal telemetry stays aligned with alliance-only aim selection by surfacing the live goal fiducial heading/range even after the lock freshness window expires, keeping the top-line “Tag Visible” readout accurate without reintroducing global tx fallbacks.
...
- **2025-12-17** – Forced AUTO to assert Limelight AprilTag pipelines without TeleOp prep, splitting obelisk observation from alliance-goal aiming. Aiming now filters strictly to goal IDs 20/24, keeps obelisk motif reads alive, surfaces pipeline/goal/obelisk status in AUTO telemetry, and adds tunables for Limelight goal vs. obelisk pipelines while preserving webcam fallback. Restored the TeleOp Limelight pipeline alias so day-of builds continue to compile after the pipeline split, and fixed the continuous-feed AutoAim nudge so it releases back to the driver’s prior toggle after the stream stops. Goal locking now pulls per-fiducial tx/tz from the alliance tag only (no global tx fallback), with stale/hysteresis/confirm tunables and telemetry for locked vs. global tx plus lock age so obelisk detections never steal aim control.
