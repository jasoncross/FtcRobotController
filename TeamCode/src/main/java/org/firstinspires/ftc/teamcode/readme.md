...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls, applied MT2 fiducial filter overrides for goal-only localization, added `rawBlue`/`rawRed` debug frame readouts plus yaw/filter age telemetry in `VisionDbg`, shifted MT2 botpose from corner-origin to center-origin by subtracting 72 in on each axis, and locked the axis mapping (`AXIS_SWAP_XY=true`, `X_SIGN=+1`, `Y_SIGN=-1`) with the transform order documented to keep the corner→center shift in field coordinates.
