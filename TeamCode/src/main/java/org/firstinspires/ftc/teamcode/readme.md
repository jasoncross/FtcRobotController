...
## Revision History
- **2025-11-26** – Hardened the Auto→TeleOp pose handoff with explicit pose-store
helpers and driver feedback: BaseAuto now records the final fused odometry pose
through the shared pose store, TeleOp seeds odometry from the saved pose during
INIT (printing the carried-over coordinates when present) before falling back to
AprilTag seeding, and documentation reflects the updated handoff.
