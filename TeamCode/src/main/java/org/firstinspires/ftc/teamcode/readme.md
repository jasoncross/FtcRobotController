...
## Revision History
- **2025-12-30** – Routed Limelight MT2 yaw feeding through LimelightHelpers’ FTC-safe static orientation calls, applied MT2 fiducial filter overrides for goal-only localization, added `rawBlue`/`rawRed` debug frame readouts plus yaw/filter age telemetry in `VisionDbg`, and updated the Limelight Y offset calibration (`Y_OFFSET_IN=118.0`) so botpose aligns with the center-origin field frame.
