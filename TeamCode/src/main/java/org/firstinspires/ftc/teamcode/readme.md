...
- FeedStop release now waits for the launcher to remain inside `SharedRobotTuning.RPM_TOLERANCE` for `SharedRobotTuning.RPM_READY_SETTLE_MS` before opening in TeleOp feeds/ejects and Auto continuous-fire steps, preventing the gate from moving forward until the flywheels are ready.
...
## Revision History
- **2025-12-31** – Allowed mixed goal+obelisk MT2 frames to fuse under cautious clamps (including obelisk-primary mixes) while still rejecting obelisk-only solves, added a two-tag guard for obelisk-primary mixes, skipped FeedStop safe-open on START to prevent a gate twitch, enforced single-source Auto odometry updates, saved the final fused pose for TeleOp handoff, cleaned up TeleOp INIT pose formatting, and gated FeedStop release until launcher RPM readiness for TeleOp feed/eject actions and Auto continuous-fire steps.
