...
tracking to use holonomic translation magnitude (preserving twist=0 behavior), restored signed-distance handling so negative mov
es drive opposite the requested heading again, renamed the test OpMode to **Auto_Test_TwistRepro** (Driver Station name **Test
 Auto: Twist Repro**), documented the behavior in the AutoSequence guide/context, and refreshed Project Layout/Revision History.
