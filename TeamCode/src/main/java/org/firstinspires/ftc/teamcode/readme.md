...
## Revision History
- **2025-12-16** – Forced AUTO to assert Limelight AprilTag pipelines without TeleOp prep, splitting obelisk observation from alliance-goal aiming. Aiming now filters strictly to goal IDs 20/24, keeps obelisk motif reads alive, surfaces pipeline/goal/obelisk status in AUTO telemetry, and adds tunables for Limelight goal vs. obelisk pipelines while preserving webcam fallback.
