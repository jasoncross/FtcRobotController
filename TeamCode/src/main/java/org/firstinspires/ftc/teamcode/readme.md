...
## Revision History
- **2025-12-16** – Smoothed Limelight goal visibility/tx for Auto aim: goal tags now stay latched through brief frame gaps with a locked tx fallback, aiming only uses alliance goal IDs 20/24 (never obelisks or best-target fallbacks), and TeleOp telemetry now shows goalSeen/smoothed/missing-frame counters to confirm sticky visibility while preserving webcam fallback and obelisk reads.
