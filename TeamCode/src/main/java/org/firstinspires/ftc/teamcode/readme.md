...
## Revision History
 - **2025-12-13** – Fixed AutoSequence move(..., twist) so translation stays field-centric while the robot yaws, swapping moveWithTwist to recompute the relative heading every loop and to track progress from encoder-derived translation distance instead of a fixed-direction multiplier. Added the `TestAuto_TwistRepro` regression OpMode (three 24" moves covering straight, twist-in-place during forward drive, and strafe + twist) plus documentation updates so teams can validate the corrected behavior and discover the new test harness in the Project Layout. Hardened Limelight polling during Autonomous tag scans by timestamping each receive locally, gating freshness against the configured max age, and surfacing resultValid/age/ids/goal telemetry so sweep loops visibly track AprilTag visibility instead of running blind on stale timestamps. Locked Limelight aiming to per-fiducial goal tx with configurable lock staleness/hysteresis and exposed locked vs. global tx in telemetry to eliminate goal/obelisk thrash during AutoAim. Restored obelisk motif memory on the Limelight path with a configurable stale timeout and new telemetry (seenThisFrame/latchedId/motif/ageMs) so motifs persist when tags leave view during Phase 3 of the retask. Added a FieldPose-aligned Limelight camera-fusion stability gate (windowed stddev/age, obelisk-only rejection, inches-mapped pose with blend alpha) plus bottom-line odometry debug blocks in TeleOp and Auto (wheel pose, raw/mapped camera pose, gate stats, tag set) so fusion only accepts stable goal-tag fixes and telemetry stays consistent with the FieldPose frame, layered a FieldPose-unit pose overlay onto the FTC Dashboard drawing so the robot icon/heading matches driver-station telemetry in inches/degrees during Phase 5, and restored SDK build compatibility by reflection-reading fiducial IDs and updating the dashboard overlay text call to the current Canvas signature.
