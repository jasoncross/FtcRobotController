...
## Revision History
- **2025-12-20** – Treated alliance-goal fiducial detections as valid for aiming/telemetry even when the Limelight result flags itself invalid, split goal detection from aim validity so telemetry shows the goal as visible whenever tag 20/24 is present while aim still requires a finite heading, and added goal-tx validity plus observed-ID reporting to the Limelight telemetry lines.
