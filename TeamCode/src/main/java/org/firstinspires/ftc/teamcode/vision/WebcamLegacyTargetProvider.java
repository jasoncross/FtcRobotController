...
public class WebcamLegacyTargetProvider implements VisionTargetProvider {
    ...
    public double getHeadingErrorDeg() {
        VisionAprilTag.AprilTagDetection det = goalDetection();
    ...
    private VisionAprilTag.AprilTagDetection goalDetection() {
        if (visionAprilTag == null) return null;
        VisionAprilTag.AprilTagDetection det = visionAprilTag.getDetectionFor(goalId());
        if (det == null) {
